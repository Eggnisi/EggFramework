...
namespace QFramework
{
    ...
    {
        bool Inited      { get; }
        bool AsyncInited { get; }
        UniTask HandleAsyncTask();
        void ExecuteAfterAsyncInitedOrInstant(Action action);
        void ExecuteInstantOrDont(Action action);
    ...
    public abstract class Architecture<T> : IArchitecture where T : Architecture<T>, new()
    {
        ...
        public bool AsyncInited { get; private set; }

        public async UniTask HandleAsyncTask()
        {
            var taskList = new List<UniTask>();
            foreach (var model in mArchitecture.mContainer.GetInstancesByType<IModel>())
            {
                taskList.Add(model.AsyncInit());
            }

            foreach (var system in mArchitecture.mContainer.GetInstancesByType<ISystem>())
            {
                taskList.Add(system.AsyncInit());
            }

            await UniTask.WhenAll(taskList);
            AsyncInited = true;
            _asyncInitAction?.Invoke();
            _asyncInitAction = null;
        }

        private Action _asyncInitAction;

        public void ExecuteAfterAsyncInitedOrInstant(Action action)
        {
            if (!AsyncInited)
            {
                _asyncInitAction += action;
            }
            else action?.Invoke();
        }

        public void ExecuteInstantOrDont(Action action)
        {
    ...
    public interface ISystem : IBelongToArchitecture, ICanSetArchitecture, ICanGetModel, ICanGetUtility,
        ICanRegisterEvent, ICanSendEvent, ICanGetSystem, ICanInit, ICanAsyncInit
    {
    }
    ...
    public abstract class AbstractSystem : ISystem
    {
        ...
        protected virtual UniTask OnAsyncInit() => UniTask.CompletedTask;
        UniTask ICanAsyncInit.AsyncInit() => OnAsyncInit();
        ...
        private Type _inst;
        protected void Log(object log) => Debug.Log($"[{_cachedType.Name}]:" + log);
        protected void LogError(object log) => Debug.LogError($"[{_cachedType.Name}]:" + log);
        protected void LogWarning(object log) => Debug.LogWarning($"[{_cachedType.Name}]:" + log);
    }
    ...
    public interface IModel : IBelongToArchitecture, ICanSetArchitecture, ICanGetUtility, ICanSendEvent, ICanInit,
        ICanGetSystem, ICanAsyncInit, ICanGetModel
    {
    }
    ...
    public abstract class AbstractModel : IModel
    {
        ...
        private Type _inst;
        protected abstract void OnInit();
        public virtual UniTask AsyncInit() => UniTask.CompletedTask;
        protected void Log(object log) => Debug.Log($"[{_cachedType.Name}]:" + log);
        protected void LogError(object log) => Debug.LogError($"[{_cachedType.Name}]:" + log);
        protected void LogWarning(object log) => Debug.LogWarning($"[{_cachedType.Name}]:" + log);
    }
    ...
    public static class BelongToArchitectureExtension
    {
        public static void ExecuteAfterAsyncInitedOrInstant(this IBelongToArchitecture self, Action action) =>
            self.GetArchitecture().ExecuteAfterAsyncInitedOrInstant(action);
        public static void ExecuteInstantOrDont(this IBelongToArchitecture self, Action action) =>
            self.GetArchitecture().ExecuteInstantOrDont(action);
    }
    ...
    public interface ICanAddAfterInitAction
    {
        ICanAddAfterInitAction AddAfterInitAction(Action action);
        void OnLateInit();
    }
